...
from services.explorer.schemas import DestinationResponse, PublicItineraryResponse
...
@router.get("/public-itineraries", response_model=dict)
...
def get_public_itinerary_detail(itinerary_id: int, db: Session = Depends(get_db)):
    ...
    if not itinerary:
        raise HTTPException(status_code=404, detail="Itinerary tidak ditemukan / tidak publik")
    return itinerary
